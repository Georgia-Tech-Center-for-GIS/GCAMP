...
function addToMap(idResults, evt) {
	...
	for (var i = 0; i < idResults.length; i++) {
		var layerDiv = dojo.create('div', {}, root);
		
		var layerTitle = null;
		var feat = idResults[i].feature;
		var attribs = feat.attributes;
		
		if (feat.geometry == null) {
			layerTitle = dojo.create('div', {
					innerHTML : idResults[i].layerName
				}, layerDiv);
		} else {
			layerTitle = dojo.create('div', {}, layerDiv);
			var layerLink = dojo.create('a', {
					href : "#",
					onClick : "showIdentifySymbol(" + i + ");",
					innerHTML : idResults[i].layerName
				}, layerTitle);
		}
	...
	dijit.byId('RightTabs').selectChild(dijit.byId('bottomIDPanel'));
	
	//map.infoWindow.show(evt.screenPoint, map.getInfoWindowAnchor(evt.screenPoint));
}

function showIdentifySymbol(index) {
	map.graphics.clear();
	lastIdResults[index].feature.setSymbol(symbol);
	
	map.setExtent(lastIdResults[index].feature.geometry.getExtent().expand(1.5));
	
	map.graphics.add(lastIdResults[index].feature);
}
