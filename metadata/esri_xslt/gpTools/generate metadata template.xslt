...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="idinfo | dataqual | spdoinfo/indspref | spdoinfo/direct | spdoinfo/ptvctinf/sdtsterm | spdoinfo/ptvctinf/vpfterm | spdoinfo/rastinfo | spref | distinfo | metainfo | MetaID | Sync | PublishedDocID | PublishStatus | Identifier | mdFileID | seqId | MemberName | catFetTyps/*[not(name() = 'genericName')] | scaleDist/uom | dimResol/uom | valUnit/*[not(name() = 'UOM')] | quanValUnit/*[not(name() = 'UOM')] | coordinates | usrDefFreq/*[not(name() = 'duration')] | exTemp/TM_GeometricPrimitive | citId/text() | citIdType | geoBox | geoDesc | MdIdent | RS_Identifier | *[@Sync = 'TRUE']" priority="1" >
	</xsl:template>
	...
	<xsl:template mode="removeEmpty"  match="*" priority="1">
		<xsl:if test="(. != '')">
			<xsl:copy>
				<xsl:apply-templates select="node() | @*" />
			</xsl:copy>
