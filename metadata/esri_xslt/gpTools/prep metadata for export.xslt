...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:esri="http://www.esri.com/metadata/">
  ...
  <!-- if gp history entry is marked for inclusion in lineage, copy and format the info in correct location -->
  <xsl:template match="dataLineage/prcStep[*/text()]" priority="1">
    <xsl:copy>
      <xsl:apply-templates select="node() | @*" />
    </xsl:copy>
  ...
  <xsl:template match="idAbs|idPurp|idCredit" priority="1">
    <xsl:copy>
      <xsl:choose>
        <xsl:when test="./*">
    ...
    <xsl:choose>
      <xsl:when test="(. = '')">
        <xsl:attribute name="gmlID">
          <xsl:value-of select="esri:GuidGen()" />
        </xsl:attribute>
        <xsl:apply-templates select="node() | @*" />
      </xsl:when>
      <xsl:otherwise>
